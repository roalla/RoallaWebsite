...
export default function HubShell({ children, userName, userEmail, role }: Props) {
  const t = useTranslations('hub')
  const pathname = usePathname()

  async function signOut() {
    await fetch('/api/auth/logout', { method: 'POST', credentials: 'include' })
    window.location.href = '/en/hub/login'
  }
    ...
    <div className="min-h-screen bg-slate-50 flex">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="md:hidden flex items-center justify-between border-b bg-white px-4 py-3">
          ...
          <button type="button" onClick={() => signOut()} className="text-sm text-slate-600">
            {t('signOut')}
          </button>
        </header>
