...
function WorkshopTopicCard({
  topic,
  t,
}: {
  topic: WorkshopTopic;
  t: ReturnType<typeof useTranslations<"workshops">>;
}) {
  return (
    <Reveal as="article" id={topic.anchor} className="scroll-mt-28 h-full">
      <div className={`${serviceCardClass} workshop-tile`}>
        <div className="p-6 lg:p-7 flex flex-col flex-1 h-full">
          ...
          <p className="text-sm text-slate-700 border-l-2 border-primary/30 pl-3 mb-4 leading-relaxed">
            {topic.ideal}
          </p>
          ...
          {topic.anchor === "ideation" ? (
            <Link href="/programs/workshops/ideation" className={servicePrimaryLinkClass}>
              {t("openTheShortlist")}
              <ArrowRight className="ml-2 w-4 h-4" />
            </Link>
          ) : null}
          <Link
            ...
            className={
              topic.anchor === "productivity" || topic.anchor === "branding" || topic.anchor === "ideation"
...
const Workshops = () => {
  ...
  const topics: WorkshopTopic[] = workshopAnchors.map((anchor, i) => ({
    ...
    features: [
      t(`topic${i}F1` as "topic0F1"),
      t(`topic${i}F2` as "topic0F2"),
      t(`topic${i}F3` as "topic0F3"),
    ],
  }));
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <Reveal className="mb-12 rounded-xl border border-slate-200 bg-slate-50 p-6 lg:p-8">
          ...
          <div className="grid sm:grid-cols-3 gap-5">
            {formatKeys.map((key, i) => (
              <div key={key} className={`${serviceMiniTileClass} workshop-tile`}>
                ...
                >
                  {React.createElement(formatIcons[i], {
                    className: "w-4 h-4 text-primary",
                  })}
                </div>
                <p className="font-semibold text-slate-900 text-sm mb-1">
                  {t(`${key}Title` as "format1Title")}
                </p>
                <p className="text-sm text-slate-600 leading-relaxed">
                  {t(`${key}Desc` as "format1Desc")}
                </p>
              </div>
            ))}
          </div>
          <div className="mt-6 border-t border-slate-200 pt-5">
            <p className="text-xs font-semibold uppercase tracking-wider text-primary-dark mb-2">
              {t("typicalEngagementTitle")}
            </p>
            <p className="text-sm text-slate-700 leading-relaxed">
              {t("typicalEngagementDesc")}
            </p>
          </div>
        </Reveal>
        ...
        <Reveal className="mt-16 pt-12 border-t border-slate-200 grid lg:grid-cols-2 gap-6">
          ...
          <div className="rounded-lg border border-slate-200 bg-slate-50 p-6 lg:p-8 flex flex-col justify-center gap-4">
            ...
            <p className="text-slate-700 leading-relaxed text-sm border-t border-slate-200 pt-4">
              {t("fitDigitalNote")}
            </p>
            <Link
