...
function Panel({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return <div className={`rounded-2xl border border-white/15 bg-white/[0.06] p-4 shadow-xl shadow-black/15 backdrop-blur-sm ${className}`}>{children}</div>;
}
  ...
  return (
    <div className="grid grid-cols-3 gap-2 p-3" aria-label={points.map((point) => point.label).join(", ")}>
      {points.map((point, index) => (
        <div key={`${point.label}-${index}`} className={`${rotations[index]} flex min-h-16 items-center justify-center rounded-md border border-amber-200/40 bg-amber-100 px-2 py-3 text-center text-sm font-semibold text-slate-900 shadow-lg`}>
          {point.label}
        </div>
      ))}
    </div>
  ...
  return (
    <div className="relative mx-auto aspect-square max-w-[21rem]" aria-label={points.map((point) => `${point.label}: ${point.detail}`).join(". ")}>
      ...
      {points.slice(0, 3).map((point, index) => {
        ...
        return (
          <div key={point.label} className={`absolute ${positions[index]} w-[42%] rounded-xl border border-white/15 bg-[#0b1b2d] p-3 text-center shadow-lg`}>
...
function Journey({ points }: { points: Point[] }) {
  return (
    <ol className="relative grid gap-2 sm:grid-cols-4">
      ...
      {points.map((point, index) => (
        <li key={point.label} className="relative rounded-xl border border-white/15 bg-[#0b1b2d]/95 p-3 sm:pt-12">
  ...
  return (
    <div className="grid grid-cols-3 gap-2" aria-label={points.map((point) => `${point.label}: ${point.detail}`).join(". ")}>
      {points.map((point, column) => (
        <div key={point.label} className={`rounded-xl border p-2 sm:p-3 ${column === 0 ? "border-brand-gold/70 bg-brand-gold/10" : "border-white/15 bg-white/[0.04]"}`}>
  ...
  return (
    <ol className="space-y-3">
      {points.map((point, index) => {
        const Icon = icons[index];
        return (
          <li key={point.label} className="flex gap-3 rounded-xl border border-white/15 bg-white/[0.05] p-3">
...
function SupportPhoto({ slide, points }: { slide: FocusSlide; points: Point[] }) {
  ...
  return (
    <div className="relative min-h-[18rem] overflow-hidden rounded-2xl border border-white/15">
      <Image src={slide.imageSrc} alt={slide.imageAlt ?? ""} fill sizes="(max-width: 768px) 100vw, 48vw" className="object-cover" />
...
function SupportCompare({ points }: { points: Point[] }) {
  return (
    <div className="grid gap-3 sm:grid-cols-2">
      {points.map((point, index) => (
        <div key={point.label} className={`rounded-2xl border p-4 ${index === 0 ? "border-emerald-300/35 bg-emerald-400/10" : "border-rose-300/30 bg-rose-400/10"}`}>
...
function Rhythm({ slide, points }: { slide: FocusSlide; points: Point[] }) {
  ...
  return (
    <div className="relative min-h-[19rem] overflow-hidden rounded-2xl border border-white/15">
      {slide.imageSrc ? <Image src={slide.imageSrc} alt={slide.imageAlt ?? ""} fill sizes="(max-width: 768px) 100vw, 48vw" className="object-cover opacity-45" /> : null}
      <div className="absolute inset-0 bg-[#07111f]/55" aria-hidden />
      <ol className="relative z-10 grid grid-cols-2 gap-2 p-4">
        {points.map((point, index) => {
          const Icon = icons[index];
          return <li key={point.label} className="rounded-xl border border-white/15 bg-[#07111f]/80 p-3 backdrop-blur-sm"><Icon className="h-5 w-5 text-brand-gold" aria-hidden /><p className="mt-2 text-sm font-semibold text-white">{point.label}</p><p className="mt-1 text-xs text-slate-300">{point.detail}</p></li>;
        })}
      </ol>
    </div>
  );
...
function Facilitator({ slide, points }: { slide: FocusSlide; points: Point[] }) {
  return (
    <div className="grid gap-3 sm:grid-cols-[8rem_1fr] sm:items-center">
      {slide.imageSrc ? <div className="relative mx-auto aspect-[4/5] w-28 overflow-hidden rounded-2xl border border-white/15 sm:w-full"><Image src={slide.imageSrc} alt={slide.imageAlt ?? ""} fill sizes="128px" className="object-cover" /></div> : null}
      <div className="space-y-2">
        {points.map((point, index) => <div key={point.label} className="flex gap-2 rounded-lg border border-white/10 bg-white/[0.04] p-2.5">{index === 0 ? <UserRoundCheck className="h-5 w-5 shrink-0 text-primary-light" aria-hidden /> : index === 1 ? <ShieldCheck className="h-5 w-5 shrink-0 text-primary-light" aria-hidden /> : <Eye className="h-5 w-5 shrink-0 text-primary-light" aria-hidden />}<div><p className="text-sm font-semibold text-white">{point.label}</p><p className="text-xs text-slate-300">{point.detail}</p></div></div>)}
      </div>
    </div>
  );
}
