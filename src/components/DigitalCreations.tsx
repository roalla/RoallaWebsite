...
import {
  ...
  type PortfolioIndustryCategoryId,
...
type IndustryFilter = "all" | PortfolioIndustryCategoryId;
...
const industryTitleKeys = {
  "fleet-logistics": "industryFleetTitle",
  "sports-recreation": "industrySportsTitle",
  "events-trade-shows": "industryEventsTitle",
  "education-training": "industryEducationTitle",
  "professional-services": "industryProfessionalTitle",
  "business-platforms": "industryPlatformsTitle",
} as const;
  ...
  return (
    <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
      {isCaseStudySlug(item.id) ? (
        <Link
          href={{
            pathname: "/services/portfolio/[slug]",
            params: { slug: item.id },
          }}
    ...
    >
      <div className="grid lg:grid-cols-2 gap-0">
        <div className="p-8 lg:p-10 flex flex-col justify-center order-2 lg:order-1">
          ...
          <p className="text-sm font-medium text-primary mb-6 border-l-2 border-primary pl-3">
            {copy.caseStudy}
          </p>
...
const DigitalCreations = ({
  ...
  useEffect(() => {
    ...
    else setCategoryFilter(initialCategory);
    ...
    if (
      portfolioIndustryCategories.some((category) => category.id === industry)
    ) {
      setIndustryFilter(industry as PortfolioIndustryCategoryId);
  ...
  const syncUrl = useCallback(
    (
      ...
      if (industry !== "all") query.set("industry", industry);
  ...
  const updateCategoryFilter = useCallback(
    (key: CategoryFilter) => {
      ...
      syncUrl(key, originFilter, industryFilter);
    },
    [industryFilter, originFilter, syncUrl],
  );

  const updateOriginFilter = useCallback(
    (key: OriginFilter) => {
      setOriginFilter(key);
      syncUrl(categoryFilter, key, industryFilter);
    },
    [categoryFilter, industryFilter, syncUrl],
  );

  const updateIndustryFilter = useCallback(
    (key: IndustryFilter) => {
      setIndustryFilter(key);
      syncUrl(categoryFilter, originFilter, key);
    },
    [categoryFilter, originFilter, syncUrl],
  );
  ...
  const featuredItems = useMemo(() => {
    let items =
      categoryFilter === "all"
        ? getFeaturedItems()
        : getFeaturedItems(categoryFilter);
    if (categoryFilter === "all" && items.length > 0) {
      const platformFeatured = items.find(
        (item) => item.featuredCategory === "platform",
      );
    ...
    return items
      ...
      .filter(
        (item) =>
          industryFilter === "all" || item.industryCategory === industryFilter,
      );
  }, [categoryFilter, industryFilter, originFilter]);
  ...
  }, [categoryFilter, originFilter, industryFilter, featuredIds]);
  ...
  const industryFilters: { key: IndustryFilter; label: string }[] = [
    { key: "all", label: t("filterIndustryAll") },
    ...portfolioIndustryCategories.map((industry) => ({
      key: industry.id,
      label: t(industryTitleKeys[industry.id]),
    })),
  ];
        ...
        <div className="mb-8 flex flex-col gap-4 rounded-xl border border-slate-200 bg-white px-4 py-4 sm:px-5 shadow-sm">
          ...
          <div className="flex flex-wrap items-center gap-2 border-t border-slate-100 pt-4">
            ...
            <div
              className="flex flex-wrap gap-2"
              role="tablist"
              aria-label={t("filterOriginLabel")}
            >
              {originFilters.map((f) => (
                <button
                  key={f.key}
          ...
          <div className="flex flex-col gap-2 border-t border-slate-100 pt-4 sm:flex-row sm:items-center">
            ...
            <select
              ...
              onChange={(event) =>
                updateIndustryFilter(event.target.value as IndustryFilter)
              }
              className="min-h-11 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/25"
            >
