...
export default function BrowserFrame({
  ...
  const frame = (
    <div className={`rounded-xl overflow-hidden border border-slate-200 shadow-card bg-white ${className}`}>
      ...
      <div className={`relative ${aspectClass} bg-slate-50 overflow-hidden group/frame`}>
        {brandPreview ? (
          ...
          <>
            ...
            />
            {href && openLabel ? (
              <span className="absolute bottom-3 left-3 right-3 flex items-center justify-center gap-1.5 rounded-lg bg-slate-900/75 text-white text-xs font-semibold py-2 px-3 backdrop-blur-sm">
                {openLabel}
              </span>
            ) : null}
          </>
