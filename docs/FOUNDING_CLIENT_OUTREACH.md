# 5-Page Website Package — Outreach Kit

Operator copy for Brevo campaigns and manual outreach. Landing page: `https://www.roalla.com/en/founding-client` (French: `/fr/founding-client`).

**Offer:** 5-page business website package · **$999 CAD** · $500 deposit / $499 on launch · 2–3 weeks · Canada-wide · English · Français

**Differentiator:** Executive-led delivery—not a template shop. Roalla is led by an executive with 30+ years across multiple industries and certifications in **ITIL, AI, and security**. Fixed scope, fixed price.

---

## English — 3-email Brevo sequence (national)

Use **Brevo Standard** for automation. Segment by city/industry when possible. Personalize `[Business Name]`, `[City]`, and `[site observation]`.

### Email 1 — Day 0

**Subject A:** `Quick question about [Business Name]'s website`  
**Subject B:** `$999 website package — 5 pages (Canada)`

> Hi [First name],  
>  
> I'm [Your name] with Roalla — we build business websites for companies across Canada. **I'm an executive with 30+ years across industries and certifications in ITIL, AI, and security** — not a freelance template shop.  
>  
> **We offer a fixed-scope 5-page website package for $999** (custom builds from $2,499+). Mobile-friendly, contact form, live in about 3 weeks.  
>  
> I noticed [site observation — e.g. hard to use on mobile / no clear way to book / looks outdated].  
>  
> **No pitch deck.** Reply with your URL or see the package here:  
> https://www.roalla.com/en/founding-client  
>  
> — [Your name], Roalla  
> Burlington, ON · English · Français  
> (289) 838-5868 · sales@roalla.com

### Email 2 — Day 3 (non-openers or no reply)

**Subject:** `Still taking website package inquiries this month`

> Hi [First name],  
>  
> Quick follow-up — our **$999 5-page website package** is still available ($500 to start, $499 on launch).  
>  
> Best for trades, clinics, restaurants, and local services that need a site that actually gets calls.  
>  
> Reply **yes** and I'll send a one-page scope — or book here:  
> https://www.roalla.com/en/schedule?intent=website&service=websites-brand&need=new&offer=founding  
>  
> — [Your name], Roalla

### Email 3 — Day 7 (last touch)

**Subject:** `Last note — [Business Name] website`

> Hi [First name],  
>  
> Last note from me — if a new site is on your list this quarter, here's what the package includes:  
> - 5 pages, mobile-friendly  
> - Contact form + click-to-call  
> - Live in 2–3 weeks  
> - $500 deposit, $499 on launch  
>  
> Details: https://www.roalla.com/en/founding-client  
>  
> — [Your name], Roalla

---

## French — Quebec / bilingual leads only

Send when the business, site, or contact is clearly French-first.

### Email 1 — Jour 0

**Objet A:** `Question rapide sur le site de [Nom de l'entreprise]`  
**Objet B:** `Forfait site 5 pages — 999 $ (Canada)`

> Bonjour [Prénom],  
>  
> Je suis [Votre nom] chez Roalla — nous créons des sites Web d'affaires pour des entreprises partout au Canada.  
>  
> **Nous offrons un forfait fixe de 5 pages pour 999 $** (sur mesure à partir de 2 499 $+). Mobile, formulaire de contact, en ligne en environ 3 semaines.  
>  
> J'ai remarqué [observation — ex. difficile sur mobile / pas de réservation claire / site daté].  
>  
> Répondez avec votre URL ou consultez le forfait :  
> https://www.roalla.com/fr/founding-client  
>  
> — [Votre nom], Roalla  
> Burlington, ON · English · Français  
> (289) 838-5868 · sales@roalla.com

### Email 2 — Jour 3

**Objet:** `Encore des demandes forfait site ce mois-ci`

> Bonjour [Prénom],  
>  
> Petit suivi — notre **forfait site 5 pages à 999 $** est toujours disponible (500 $ pour commencer, 499 $ au lancement).  
>  
> Idéal pour les entreprises locales qui ont besoin d'un site qui génère des appels.  
>  
> Répondez **oui** et j'envoie un mandat d'une page — ou réservez ici :  
> https://www.roalla.com/fr/schedule?intent=website&service=websites-brand&need=new&offer=founding  
>  
> — [Votre nom], Roalla

---

## LinkedIn DM (English)

> Hi [Name] — I build 5-page business websites for Canadian SMBs. Fixed-scope package: **$999**, live in ~3 weeks. Saw [Business] — happy to send a one-page scope if useful. https://www.roalla.com/en/founding-client

---

## Phone script (15 seconds)

> "Hi, I'm [Name] from Roalla in Burlington — we build websites for businesses across Canada. We have a five-page website package at nine ninety-nine. Is updating your site something you're looking at this year?"

---

## Niche observation examples

| Industry | Observation |
|---|---|
| Trades (HVAC, plumber) | "On mobile, the call button is hard to find and the site loads slowly." |
| Dental / physio | "The site doesn't show online booking or clear new-patient info above the fold." |
| Restaurant | "The menu isn't easy to read on a phone and hours aren't obvious." |
| Professional services | "The site looks dated compared to competitors — might affect trust for new clients." |

---

## Tracking (simple sheet)

| # | Business | City | Industry | Email sent | Reply | Deposit | Launch | Review |
|---|---|---|---|---|---|---|---|---|
| 1 | | | | | | | | |

**Goal:** 30–40 outreaches/day · 5–7 deposits/month · ~$10k/mo at ~10 sites/month

---

## After deposit — scope one-pager

Send same day. See package page for full inclusions/exclusions: `/en/founding-client`

**Payment:** $500 deposit (non-refundable once work begins) · $499 on launch  
**Optional:** Google review and client example permission (confirm in scope doc, not required on marketing page)
