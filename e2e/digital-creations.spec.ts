...
test.describe('Digital creations portfolio order', () => {
  test('hero, jump nav, and all projects list platforms first', async ({ page }) => {
    ...
    await expect(heroLinks.nth(3)).toContainText('coldbru.dejabru.ca')
    await expect(heroLinks.nth(4)).toContainText('roalla.com')
    ...
    await expect(jumpLinks.nth(3)).toHaveText(/Cold Deja Bru/i)
    await expect(jumpLinks.nth(4)).toHaveText(/ROALLA/i)
    ...
    await expect(projectCards.nth(2)).toHaveText(/Cold Deja Bru/i)
    await expect(projectCards.nth(3)).toHaveText(/ROALLA/i)
  })
})
