...
test.describe('Digital creations portfolio order', () => {
  test('hero, jump nav, and all projects list platforms first', async ({ page }) => {
    ...
    await expect(heroLinks.nth(2)).toContainText('k3green-production.up.railway.app')
    await expect(heroLinks.nth(3)).toContainText('soaringpuck.com')
    await expect(heroLinks.nth(4)).toContainText('keneffect.com')
    await expect(heroLinks.nth(5)).toContainText('coldbru.dejabru.ca')
    ...
    await expect(jumpLinks.nth(2)).toHaveText(/Pulsavant Solutions/i)
    await expect(jumpLinks.nth(3)).toHaveText(/Soaring Puck/i)
    await expect(jumpLinks.nth(4)).toHaveText(/Ken Effect/i)
    await expect(jumpLinks.nth(5)).toHaveText(/Cold Deja Bru/i)
    ...
    await expect(projectCards.nth(1)).toHaveText(/Pulsavant Solutions/i)
    await expect(projectCards.nth(2)).toHaveText(/Soaring Puck/i)
    await expect(projectCards.nth(3)).toHaveText(/Ken Effect/i)
    await expect(projectCards.nth(4)).toHaveText(/Cold Deja Bru/i)
