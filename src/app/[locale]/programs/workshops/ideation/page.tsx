...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const copy = companionWorkshopCopy("ideation", locale);
  return buildPageMetadata({ locale, path: copy.path, title: copy.metaTitle, description: copy.metaDescription });
}

export default async function IdeationWorkshopPage({ params }: Props) {
  const { locale } = await params;
  const copy = companionWorkshopCopy("ideation", locale);
  ...
  return <div className="page-shell"><Script id="ideation-jsonld" type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} /><div className="h-1 bg-gradient-to-r from-transparent via-primary to-transparent" aria-hidden /><div className="container mx-auto px-4 pb-16 pt-24 sm:px-6 lg:px-8 lg:pt-28"><CompanionWorkshopExperience copy={copy} /></div></div>;
}
